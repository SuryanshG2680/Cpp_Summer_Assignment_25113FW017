...
void issue(vector<bookinfo> &book, vector<issuedbooks> &issued);
void returnbook(vector<bookinfo> &book, vector<issuedbooks> &issued);
void availability(const vector<bookinfo> &book);
...
int main(){
    ...
    vector<struct bookinfo> books;
    vector<issuedbooks> issued;
    int c=0;
    char available, add;
    ...
    do{
        ...
        switch(option){
            ...
            case 6 : updateinfo(books);
                    break;
            case 7 : issue(books, issued);
                    break;
            case 8 : returnbook(books, issued);
                    break;
            case 9 : availability(books);
                    break;
            case 10 : cout << "\n~~~~~~ THANKYOU ~~~~~\n";
                    return 0;
...
void sorting(vector<bookinfo> &book){
    ...
    do{
        ...
        switch(option){
            ...
            case 3 : sort(book.begin(), book.end(), cmpyear); 
                    cout << "***** Sorting by Year Done \n";
                    break;
    
            case 4 : sort(book.begin(), book.end(), cmpauthor); 
                    cout << "***** Sorting by Name of Author Done \n";
                    break;
    ...
    do{
        ...
        switch (option){
            ...
            case 7 : cout << "Enter Number of books available : ";
                    ...
                    break;
            case 8 : cout << "Is the book Available? (y/n) ";
                    cin >> yesno;
                    if(yesno == 'y' || yesno == 'Y' ) book[i].available = true;
                    else if (yesno == 'n' || yesno == 'N') book[i].available = false;
...
void issue(vector<bookinfo> &book, vector<issuedbooks> &issued){
    ...
    if(book[i].available) {
        cout << book[i].number << " books available\n";
        cout << "Enter Issue Date : ";
        cin >> issuedate;
        cout << "Book Issued\n";
        book[i].number--;
        book[i].number==0? book[i].available = false : book[i].available = true;
    }
    else  cout << "Not Available\n";
    issued.push_back(temp);
...
void returnbook(vector<bookinfo> &book, vector<issuedbooks> &issued){
    ...
    for(int j=0; j<issued.size(); j++){
        if(issued[j].studentid == studentid && issued[j].bookid == returnid && issued[j].returned == false){
            cout << "Name = " << book[i].name << endl;
            cout << book[i].number << " books available\n";
            cout << "1 Book Returned\n";
            issued[j].returned = true;
            book[i].number++;
...
void availability(const vector<bookinfo> &book){
    ...
    if(book[i].available) cout << "The book is available\n";
